...
fn parse_usize(i: Option<&str>) -> Result<usize, MyError> {
    ...
    match j.parse::<usize>() {
        Ok(k) => Ok(k),
        Err(_) => Err(MyError::ParseIntError {integer: j.to_string()}),
    }
}
...
fn parse_file(path: Option<&str>) -> Result<PathBuf, MyError> {
    ...
    if pb.is_file() {
        ...
        Err(MyError::PathNotExistError { path: spath.to_string() })
    }
}
...
impl Config for WindowConfig {
    ...
    fn parse_clap(app: &ArgMatches<'static>) -> Result<Box<Self>, MyError> {
        ...
        let config = Self { fasta: fasta, window: window, step: step };
        Ok(Box::new(config))
    }
}
...
impl Config for SNPConfig {
    ...
    fn parse_clap(app: &ArgMatches<'static>) -> Result<Box<Self>, MyError> {
        ...
        let config = Self { fasta: fasta, vcf: vcf };
        Ok(Box::new(config))
    }
}
